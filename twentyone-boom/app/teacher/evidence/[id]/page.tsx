  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="card mt-6">
        ...
        {evidence.fileUrl && evidence.fileType === "image" && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={evidence.fileUrl} alt="bewijs" className="max-h-96 rounded-md border border-line" />
        )}
        {evidence.fileUrl && evidence.fileType === "pdf" && (
          <iframe src={evidence.fileUrl} className="w-full h-96 rounded-md border border-line" />
        )}
        {evidence.fileUrl && evidence.fileType === "video" && (
          <video src={evidence.fileUrl} controls className="max-h-96 rounded-md border border-line" />
        )}
        {evidence.linkUrl && (
          <a href={evidence.linkUrl} target="_blank" rel="noopener noreferrer" className="underline text-forest">
            🔗 {evidence.linkUrl}
          </a>
        )}
        {!evidence.fileUrl && !evidence.linkUrl && (
