...
async function main() {
  ...
  // ----- Tips -----
  // Wipe dismissed-tips first because they FK to tips
  await prisma.dismissedTip.deleteMany();
  ...
  await prisma.user.upsert({
    where: { email: "admin@school.nl" },
    create: {
      email: "admin@school.nl",
      name: "Schoolcoördinator",
      passwordHash: hash,
      role: "ADMIN",
    },
    update: {},
  ...
  console.log("  Admin:     admin@school.nl  /  welkom123");
}
